...
- **DSN 经环境变量 `DATABASE_SOURCE` 注入**：`NewData` 用 `os.Getenv` 直接读取。初版用 `config.yaml` 的 `${DATABASE_SOURCE}` 占位 + `os.ExpandEnv`，但实测 Kratos config 把 `${VAR}` 替换为空（其 env 替换不读 godotenv 注入的进程环境），故改为直接 `os.Getenv`、不经 `config.yaml`。明文密码不入库、不入配置。开发时用 `.env`（`godotenv` 加载到环境变量，`.gitignore` 已排除，模板 `.env.example` 入库）；生产可改用真实环境变量或 secret manager。
...
- **代价**：
  ...
  - `make run` 现依赖本地 MySQL 可达（`scripts/init.sql` 建库；表由 ent 自动建），不再是「零外部依赖」demo。
